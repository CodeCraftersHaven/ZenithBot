...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Client, Embed, EmbedBuilder, Message, TextChannel } from "discord.js";
...
export default class Giveaways {
    ...
    async addParticipantToDb(messageId: string, participant: Entry): Promise<void> {
    ...
    async removeParticipantFromDb(messageId: string, participant: Entry): Promise<void> {
        ...
        if (giveaway) {
            ...
            const updatedParticipants = giveaway.entries.filter((p) => p.userId !== participant.userId);
            await this.db.giveaway.update({
                where: { messageId },
    ...
    async getGiveaway(messageId: string) {
        const giveaway = await this.db.giveaway.findUnique({ where: { messageId } })!;
        return giveaway;
    }
    ...
    async deleteGiveaway(messageId: string, userId: string) {
        ...
        const channel = await this.c.channels.fetch(giveaway.channelId) as TextChannel;
    ...
    async rerollWinners(messageId: string): Promise<string[]> {
        ...
        const winnersField = embed.fields.find(f => f.name.toLowerCase().includes("winners"));
        const endingField = embed.fields.find(f => f.name.toLowerCase() === "\u200b");
        if (winnersField && endingField) {
            ...
            const claimRow = [new ActionRowBuilder<ButtonBuilder>({ components: [claimButton, reRollButton] })];
    ...
    async createTimers(interval: number, restart: boolean = false, options: Opts) {
        ...
        return setTimeout(async () => {
            const giveawayWinners = await this.drawWinnersFromDb(options.messageId, options.winnerSize);
            const channel = await this.c.channels.fetch(options.channelId) as TextChannel;
            const msg = await channel?.messages.fetch(options.messageId).catch(() => null);
            if (!msg) {
                this.log(`Message with ID ${options.messageId} not found.`);
                await this.db.giveaway.delete({ where: { messageId: options.messageId}})
                return;
            }
            ...
            if (winnersField) {
                ...
                if (!giveawayWinners || giveawayWinners.length === 0) {
                    const newFields = embed.fields.filter(f => f.name.toLowerCase() !== "winners" && !f.name.toLowerCase().includes(" "));
                    newEmbed.setTitle("Giveaway has Ended!");
                    newEmbed.setFields(newFields);
                    await msg.edit({ embeds: [newEmbed], components: [] }).then((m) => setTimeout(async () => {
