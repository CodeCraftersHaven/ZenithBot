...
export default commandModule({
  ...
  async execute(ctx) {
    ...
    if ((message.system || !message.content) && !message.embeds.length) {
       const notAMessage = await translateText(`There is nothing to translate.`, defaultLang.value);
    ...
    if (message.embeds.length > 0) {
        for (const embed of message.embeds) {
            ...
            if (embed.title) newEmbed.setTitle(await translateText(embed.title, defaultLang.value));
            if (embed.description) newEmbed.setDescription(await translateText(embed.description, defaultLang.value));
            if (embed.author) {
                const name = await translateText(embed.author.name, defaultLang.value);
                newEmbed.setAuthor({ name, iconURL: embed.author.iconURL ?? undefined, url: embed.author.url ?? undefined });
            }
            if (embed.footer) {
                const text = await translateText(embed.footer.text, defaultLang.value);
                newEmbed.setFooter({ text, iconURL: embed.footer.iconURL ?? undefined });
            }
            
            if (embed.fields?.length) {
                const newFields = await Promise.all(embed.fields.map(async (field) => ({
