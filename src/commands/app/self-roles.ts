...
export default commandModule({
      ...
      options: [
        {
          ...
          command: {
            ...
            async execute(ctx, tbd) {
              ...
              const messages = await selfRoles.findMany({
                where: { guildId: ctx.guildId! },
                select: { messages: true },
              });
              return messages.flatMap(({ messages }) =>
                messages.map((msg) => ({ name: msg.title, value: msg.id })),
              );
            },
          },
        },
        {
          ...
          command: {
            ...
            async execute(ctx, tbd) {
              ...
              return roles
                .flatMap(({ messages }) =>
                  messages.flatMap(({ roles }) => roles),
                )
                .map(({ roleId, name }) => ({ name, value: roleId }));
            },
          },
        },
      ],
  ...
  async execute(ctx, { deps }) {
    const [prisma, { SelfRoles }, client, logger] = [
      deps["@prisma/client"],
      deps["systems"],
      deps["@sern/client"],
      deps["@sern/logger"],
    ...
    const enabled = await prisma.systems.findFirst({
      where: {
        id: ctx.guild?.id!,
        systems: { some: { name: "SelfRoles".toLowerCase(), enabled: true } },
      },
    });
    if (!enabled) {
      new SelfRoles(false);
      return ctx.reply(
        "The SelfRoles system is not enabled in this server. Please use </system enable:1335686173011476501>",
      );
    }
    const { selfRoles } = prisma;
    // const subs = {
