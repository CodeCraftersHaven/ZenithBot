...
import { ActionRowBuilder, ApplicationCommandOptionType, ButtonBuilder, ButtonStyle, ChannelType, Colors, EmbedBuilder, MessageFlags, PermissionsBitField, TextChannel } from "discord.js";
...
export default commandModule({
            ...
            options: [
                {
                    ...
                    command: {
                        async execute(ctx, { deps }) {
                            ...
                            const filter = (await giveaways.findMany({ where: { guildId: ctx.guildId! } }))
                            .filter(x => x.messageId.startsWith(focusedOption))
                            .map(u => ({ name: `${u.prize}-${u.id}`, value: u.messageId }))
    ...
    async execute(ctx, { deps }) {
        const [prisma, { Giveaway }, client, logger] = [deps["@prisma/client"], deps["systems"], deps["@sern/client"], deps["@sern/logger"]];
        const enabled = await prisma.systems.findFirst({ where: { id: ctx.guild?.id!, systems: { some: { name: "giveaways", enabled: true } } } });
        if (!enabled) {
            new Giveaway(false)
            return ctx.reply("The giveaways system is not enabled in this server. Please use </system enable:1335686173011476501>");
        }
        const giveaway = new Giveaway(true);
        const channelId = enabled.systems.find((sys) => sys.name === "giveaways")?.channels[0].id!;
        ...
        const subcommands = {
            create: async () => {
                ...
                if (!part1 || !part1.split(" ")[0] || isNaN(Number(part1.split(" ")[0]))) {
                    return ctx.reply("Invalid time format. Using `and` more than once is invalid and won't be used. Valid formats: `1 sec | secs | second | seconds`, `1 min | mins | minute | minutes`, `1 hour | hours | hr | hrs`, `1 day | days`, `1 hour and 30 mins`, `2 days and 3 hours`");
                }
                ...
                const buttons = ["🎉|Enter Giveaway", "🔨|Leave Giveaway", "❌|End Giveaway"].map(button => {
                ...
                const giveawayChannel = client.channels.cache.find((c) => c.type == ChannelType.GuildText && c.id === channelId)!;
                if (!giveawayChannel) {
                    logger.error("No giveaway channel found.");
                    return ctx.reply("No giveaway channel found.");
                }
                await (giveawayChannel as TextChannel).send({ embeds: [embed], components: [row] }).then(async (msg) => {
                ...
                return ctx.reply({ content: "Giveaway created.", flags: MessageFlags.Ephemeral });
            },
            list: async () => {
                if (!ctx.guild) {
