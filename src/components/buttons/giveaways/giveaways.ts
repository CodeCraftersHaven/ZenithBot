...
import { TextChannel, ChannelType, MessageFlags, ButtonBuilder, ButtonStyle, ActionRowBuilder, Message, Embed, APIEmbedField, User, Collection, EmbedBuilder } from "discord.js";
...
export default commandModule({
    ...
    async execute(ctx, { deps, params }) {
        const act = params! as 'claim' | 'close' | 'enter' | 'leave' | 'end' | 'reroll';
        ...
        const findFields = (field: string, embed: Embed = giveawayMessage.embeds[0]) => {
            return embed.fields.find(f => f.name.toLowerCase().includes(field)) as APIEmbedField
        }
        const findFieldsValues = (field: string, embed: Embed = giveawayMessage.embeds[0]): string => {
            return (embed.fields.find(f => f.name.toLowerCase().includes(field)) as APIEmbedField).value
        }
        const acts = {
            claim: async () => {
                ...
                function extractUserIds(input: string | string[]) {
                    ...
                    if (Array.isArray(input)) {
                        return input.flatMap(str => [...str.matchAll(userIdRegex)].map(match => match[1] || match[0]));
                    } else if (typeof input === "string") {
                        return [...input.matchAll(userIdRegex)].map(match => match[1] || match[0]);
                    }
                ...
                const newChannel = async (): Promise<TextChannel> => {
                    return await ctx.guild?.channels.create({
                        ...
                        permissionOverwrites: [
                            ...
                            { id: ctx.client.user.id, allow: ["ViewChannel", "SendMessages", "ManageChannels"] }
                        ],
                        type: ChannelType.GuildText
                    }) as TextChannel;
                }
                if (!winners.includes(winnerId)) {
                    return await ctx.reply({ flags: MessageFlags.Ephemeral, content: "This isn't your button!" })
                } else {
                    ...
                    await newChannel().then(async (c) => {
                        const rep = await currentChannel.send({ content: `<@${winnerId}> Here's your claim channel: <#${c?.id}>` });
                        const close = new ButtonBuilder({
                            custom_id: "giveaways/close",
                            label: "Close Channel",
                            style: ButtonStyle.Primary
                        });
                        const row = new ActionRowBuilder<ButtonBuilder>({ components: [close] })
                        c?.send({ content: `<@${host}>, <@${winnerId}> is here to claim their prize (${prize})! When the claim process is complete, please click "Close Channel" below to delete this channel!`, components: [row] });
                        await ctx.deleteReply()
                        const filter = (m: Message) => m.author.id === winnerId;
                        const collector = c.createMessageCollector({ filter, time: 86400000, max: 1 }); // 24 hours
                        ...
                        collector.on('end', async (collected) => {
                            if (collected.size === 0) {
                                c.send(`<@${winnerId}> did not send any messages in the claim channel for 24 hours.`).catch((_) => { });
            ...
            enter: async () => {
                ...
                if (ctx.user.id === hostUserId) {
                    return ctx.editReply({ content: `Bruh this is your giveaway! You can't win it too!` })
                }

                g.addParticipantToDb(giveawayMessage.id, { userId: ctx.user.id })
                const entriesField = giveawayMessage.embeds[0].fields.find(field => field.name === 'Entries:');
                if (entriesField) {
            ...
            leave: async () => {
                ...
                if (ctx.user.id === hostUserId) {
                    return ctx.editReply({ content: `Bruh this is your giveaway! You can't leave when you can't even enter!` })
                }
                ...
                let newEmbed = new EmbedBuilder(EmbedBuilder.from(giveawayMessage.embeds[0]).toJSON());
                const newFields = giveawayMessage.embeds[0].fields.filter(() => !ending && !winners)
                newEmbed.setTitle("Giveaway Ended!")
                newEmbed.setFields(newFields)
                newEmbed.setFooter({text: "Host Ended Giveaway"})
                await giveawayMessage.edit({ embeds: [newEmbed], components: [] }).then((m) => {
                    setTimeout(async() => {
                        await m.delete()
                    }, 5000);
                })
