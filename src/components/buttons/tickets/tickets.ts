...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, Guild, MessageFlags, PrivateThreadChannel, TextChannel, User } from "discord.js";

export default commandModule({
    type: CommandType.Button,
    async execute(ctx, { deps, params }) {
        await ctx.deferReply({ flags: MessageFlags.Ephemeral })
        const [tickets, db, logger] = [deps["systems"].Tickets, deps["@prisma/client"], deps["@sern/logger"]];
        ...
        const acts = {
            open: async () => {
                ...
                const closeTicket = new ButtonBuilder()
                    ...
                    .setEmoji("🔒");
                const closeRow = new ActionRowBuilder<ButtonBuilder>({ components: [closeTicket] });
                await newThread.send({ content: `<@${user.id}>, here is your ticket channel. Please briefly describe your issue and a moderator will be with you shortly.`, components: [closeRow] })

                await Ticket.openTicket()
                return ctx.editReply(`Ticket opened in <#${(newThread as PrivateThreadChannel).id}>`);
            },
            close: async () => {
                ...
                await channel.send({ content: `The channel has been locked. Only mods can reopen the ticket if needed..` })
            ...
            reopen: async () => {
                ...
                return await ctx.editReply("I've reopened the channel for you. Do you still need help?")
            },
            check: async () => {
                const channel = ctx.message.channel as TextChannel;
                const Ticket = new tickets(true, guild.id, channel.id, user.id);
                ...
                if (ticketExists.exists) {
                    ...
                    return await ctx.editReply(`You have a ticket open here: <#${ticket?.channelId}> `)
                }
