...
export default commandModule({
    ...
    async execute(ctx, { deps, params }) {
        ...
        const [AntiScam, prisma] = [deps["systems"].AntiScam, deps["@prisma/client"]];
        const operator = ctx.member as GuildMember;
        const userId = ctx.message.embeds[0].fields.find((f) => f.name === "User")?.value.split(" ")[1].replace("(", "").replace(")", "");
