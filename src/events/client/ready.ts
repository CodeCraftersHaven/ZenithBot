...
export default eventModule({
    ...
    async execute(client) {
        const [logger, prisma, { Giveaway }, c] = Services("@sern/logger", "@prisma/client", "systems", "@sern/client");
        ...
        for (const data of db) {
            if (data) {
                try {
                    ...
                    logger.error(`Failed to fetch message with ID ${data.messageId}: ${error}: Deleting from Database...`);
                    await prisma.giveaway.deleteMany({ where: { messageId: data.messageId } })
                }
            }
        }
