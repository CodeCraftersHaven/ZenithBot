...
import { AuditLogEvent, Events, Message, MessageCreateOptions, MessagePayload, TextChannel } from "discord.js";
...
export default eventModule({
    ...
    async execute(guild) {
        await guild.fetch()
        const [logger, prisma, c] = Services("@sern/logger", "@prisma/client", "@sern/client");
        ...
        const systems = fs.readdirSync(systemsFolder).map(async (file: string) => {
            ...
            const systemName = systemModule.default?.name || path.basename(file, path.extname(file));
            return systemName as string;
        });
        const systemsList = await Promise.all(systems);
        const filteredSystems = systemsList.filter(f => f != 'index' && f != 'Systems');
        ...
        if (entry && entry.executor) {
            logger.info(`Bot was invited by: ${entry.executor.tag} (ID: ${entry.executor.id})`);
        } else {
        ...
        const embed = {
            ...
            fields: [
                ...
                {
                    name: "Invite Me",
                    value: "[Invite](https://discord.com/oauth2/authorize?client_id=1333971772193771530)"
                },
                {
                    name: "Source Code",
        ...
        async function sendMessageWithRetry(channel: TextChannel, message: string | MessagePayload | MessageCreateOptions, retries = 5, delay = 60000) {
            for (let attempt = 1; attempt <= retries; attempt++) {
                try {
                    await channel.send(message);
                    logger.info(`Message sent successfully in #${channel.name} (Attempt ${attempt})`);
        ...
        try {
            ...
            entry?.executor?.createDM(true).then((c) => c.send({
