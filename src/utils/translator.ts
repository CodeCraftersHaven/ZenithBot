...
async function translateImage(imageUrl: string, target: string) {
  try {
    ...
    let blocks = pageData.paragraphs || (pageData.blocks || []).flatMap((b: any) => b.paragraphs) || [];
    
    if (blocks.length === 0 && pageData.lines && pageData.lines.length > 0) {
    ...
    if (blocks.length === 0) {
        ...
        const attachment = new AttachmentBuilder(buffer, { name: "translated_image.png" });
