...
const locationsFn = (map: ExpiryMap<string, number>) =>
  ({
    [CooldownLocation.channel]: (value) =>
      __add(map, [[CooldownLocation.channel, value]]),
    [CooldownLocation.user]: (value) =>
      __add(map, [[CooldownLocation.user, value]]),
    [CooldownLocation.guild]: (value) =>
      __add(map, [[CooldownLocation.guild, value]]),
...
export const cooldown2 = () => {
  ...
  return {
    add: (
      ...
      >,
      message?: CooldownResponse,
    ) => __add(cooldownMap, items, message),
